...
def test_real_customer_approves_locks_and_reopens_exact_prd(tmp_path):
    ...
    application = AuthenticatedCustomerApplication(
        authentication,
        workspace,
        preauth_secret=b"day-16-founder-browser-preauthentication-secret",
    ...
    try:
        with playwright.sync_playwright() as runtime:
            ...
            page.get_by_role(
                "heading",
                name=(
                    "Approve and lock Community workshop planner — "
                    "Product Requirements Document"
                ),
            ).wait_for()
            assert page.get_by_text("does not create a roadmap").is_visible()
            page.get_by_text(
            ...
            assert page.get_by_text("LOCKED", exact=True).is_visible()
            ...
            assert page.get_by_text(
                "Locked scope — planning and implementation have not started"
            ).is_visible()
            ...
            assert page.get_by_text("Every schedule update is visible").first.is_visible()
    ...
    _, draft, requirements_approval, prd, prd_approval = prd_approvals.context(
        customer_id,
        product.request_id,
    )
    ...
    target = os.environ.get("ASCOS_DAY16_FOUNDER_EVIDENCE_DIR")
    if target:
        _write_founder_evidence(
            Path(target),
            product.digest,
            draft.digest,
            requirements_approval.digest,
            prd.digest,
            prd_approval.digest,
            screenshot,
        )
...
def _write_founder_evidence(
    ...
    screenshot_path = target / "prd-approved-and-locked.png"
    screenshot_path.write_bytes(screenshot)
    manifest = {
        "schema_version": 1,
        "result": "PASS",
        "journey": "customer.prd.approve_and_lock",
        "claims": [
            "AUTHENTICATED_CUSTOMER_OWNS_EXACT_PRD",
            "EXPLICIT_PRD_CONFIRMATION_CAPTURED",
            "EXACT_PRD_DIGEST_BOUND",
            "PRD_APPROVAL_RECEIPT_WRITE_ONCE",
            "GOVERNED_PRD_STATUS_LOCKED",
            "EVERY_REQUIREMENT_STATUS_LOCKED",
            "RETURNING_CUSTOMER_REOPENED_LOCKED_PRD",
            "NO_ROADMAP_AGENT_REPOSITORY_OR_IMPLEMENTATION_AUTHORITY",
