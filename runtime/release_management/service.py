...
class ReleaseManagementService:
    def __init__(self, store: ReleaseStore, project_registry=None, event_publisher=None) -> None:
    ...
    def approve(self, release: Release, approval: ReleaseApproval, now: datetime) -> Release:
    ...
    def generate_notes(release: Release) -> ReleaseNotes:
        return ReleaseNotes(f"Release {release.version}",f"Deterministic release {release.version}",
                            tuple(sorted(release.requirement_ids)),tuple(sorted(release.milestone_ids)),
                            tuple(sorted(release.commit_shas)),tuple(sorted(release.pull_request_urls)),
                            tuple(sorted(release.decision_ids)))
